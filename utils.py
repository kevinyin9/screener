...
def setup_logger(name, *, is_verbose=False, is_file_handled=False, file_dir=None):
    ...
    color_formatter = ColorFormatter(
        format='[%(asctime)s.%(msecs)03d] [%(levelname)s][%(filename)s:%(lineno)s - %(funcName)20s()] %(message)s',
        datefmt='%Y-%m-%d %H:%M:%S',
    ...
    if is_file_handled:
        normal_formatter = logging.Formatter(
            '[%(asctime)s.%(msecs)03d] [%(levelname)s][%(filename)s:%(lineno)s - %(funcName)20s()] %(message)s',
            datefmt='%Y-%m-%d %H:%M:%S')
