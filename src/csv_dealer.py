...
class CsvDealer:
    def __init__(
        self, file_path, logger, strategy_name, date_format="%Y-%m-%d", open_file=True
    ):
    ...
    def get_last_row_date_csv(self, format="%Y%m%d"):
        try:
            with open(
                self._file_path, "r", encoding="utf-8", errors="ignore"
            ) as scraped:
                last_row = None
                ...
                if last_row[0] == 'datetime': # It means this pair has no any data.
                    return None
                return (
                    datetime.strptime(last_row[0], format)
    ...
    def write_if_needed(self, msg, last_update_date):
        if isinstance(last_update_date, str):
            try:
                last_update_date = datetime.strptime(
                    last_update_date, self._date_format
                )
            except ValueError:
        ...
        if isinstance(msg, str):
            data_dt = (
                datetime.strptime(msg.split(",")[0], self._date_format)
                if last_update_date is not None
                else None
            )
            if (
                data_dt is None
                or last_update_date is None
                or last_update_date < data_dt
            ):
            ...
            if isinstance(data_dt, str):
                data_dt = (
                    datetime.strptime(msg[0], self._date_format)
                    if last_update_date is not None
                    else None
                )
            if (
